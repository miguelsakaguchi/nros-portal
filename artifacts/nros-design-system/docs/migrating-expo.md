# Migrating Expo UI to NROS Design System

Read `artifacts/nros-design-system/docs/AGENTS.md` and
`artifacts/nros-design-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/nros-design-system/lib/native-theme` and
  `@workspace/nros-design-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/nros-design-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/nros-design-system` and retains only
