...
export function OverviewPage() {
  return (
    <div className="space-y-6">
      <section className="rounded-xl border border-primary/20 bg-primary/5 p-6 text-card-foreground">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Dados → análise → ação</p>
        <h2 className="mt-3 max-w-2xl text-2xl font-semibold tracking-tight">
          Clareza executiva para cuidar da saúde psicossocial com responsabilidade.
        </h2>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">
          O NROS combina segurança, inteligência e linguagem acessível. A interface
          orienta a leitura rápida sem alarmismo e transforma recomendações em próximos passos.
        </p>
      </section>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="rounded-xl border bg-card p-5 text-card-foreground">
          ...
          <Card className="mt-4">
            <CardHeader>
              <CardTitle>Iniciar avaliação</CardTitle>
              <CardDescription>
                Componentes compostos a partir dos tokens NROS.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-1.5">
                <Label htmlFor="overview-name">Nome da empresa</Label>
                <Input id="overview-name" placeholder="Ex.: Acme Brasil" />
              </div>
              <div className="flex items-center gap-2">
                <Switch defaultChecked id="overview-notify" />
                <Label htmlFor="overview-notify">Notificar gestores</Label>
                <Badge className="ml-auto">Ativo</Badge>
              </div>
            </CardContent>
            <CardFooter className="gap-2">
              <Button>Continuar</Button>
              <Button variant="outline">Salvar depois</Button>
            </CardFooter>
          </Card>
        </section>
      </div>
